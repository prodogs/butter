...
define(["util/lang", "text!layouts/super-scrollbar.html"],
    function (LangUtils, SUPER_SCROLLBAR_LAYOUT)
    {
            ...
            media)
        {
            ...
            {
                if (_viewPort.getBoundingClientRect()
                    .width < ARROW_MIN_WIDTH)
                {
            ...
            {
                e.stopPropagation();
                media.currentTime = (e.clientX - _rect.left) / _rect.width *
                    _duration;
            ...
            onViewMouseMove = function (e)
            {
                e.preventDefault();
                e.stopPropagation();
                _boundsChangedCallback(Math.max(0, (e.clientX - _rect.left -
                    _offset)) / _rect.width, -1);
            };
            ...
            onLeftMouseMove = function (e)
            {
                ...
                {
                    position = _viewPort.offsetLeft + _viewPort.clientWidth -
                        MIN_WIDTH;
                }

                _viewPort.style.left = position / _rect.width * 100 + "%";
                _boundsChangedCallback(_viewPort.offsetLeft / _rect.width,
                    _viewPort.offsetWidth / _rect.width);
            };
            ...
            updateView = function ()
            {
                _viewPort.style.left = outerElement.scrollLeft / innerElement.offsetWidth *
                    100 + "%";
                _viewPort.style.right = (1 - (outerElement.scrollLeft +
                    outerElement.offsetWidth) / innerElement.offsetWidth) * 100 +
                    "%";
            };
            ...
            function scaleViewPort(scale)
            {
                ...
                rightPosition = (1 - ((viewLeft + viewWidth) / rectWidth)) +
                    halfScale;
            ...
            function zoomSliderMouseUp()
            {
                ...
                window.removeEventListener("mousemove", zoomSliderMouseMove,
                    false);
                _zoomSliderContainer.addEventListener("mousedown",
                    zoomSliderContainerMouseDown, false);
                _zoomSliderHandle.addEventListener("mousedown",
                    zoomSliderHanldeMouseDown, false);
            }
            ...
            {
                var position = _zoomSlider.offsetWidth - (e.clientX - (
                    _zoomSliderContainer.offsetLeft + (_zoomSliderHandle.offsetWidth /
                        2))),
                    scale;
                ...
                scaleViewPort(scale);
                _zoomSliderHandle.style.right = position / _zoomSlider.offsetWidth *
                    100 + "%";
            }
            ...
            function zoomSliderContainerMouseDown(e)
            {
                ...
                updateZoomSlider(e);
                _zoomSliderHandle.removeEventListener("mousedown",
                    zoomSliderHanldeMouseDown, false);
                _zoomSliderContainer.removeEventListener("mousedown",
                    zoomSliderContainerMouseDown, false);
            ...
            function updateTrackEventVisual(trackEvent, order)
            {
                ...
                trackEventVisual.classList.add(
                ...
                trackEventVisual.style.top = (trackEventVisual.offsetHeight +
                    TRACK_PADDING) * order + "px";
            }
            ...
            _media.listen("trackorderchanged", function (e)
            {
                ...
                for (; i < il; i++)
                {
                    ...
                    {
                        trackEvent = _trackEventVisuals[track.trackEvents[j]
                            .id];
                        if (trackEvent)
                        {
                            trackEvent.style.top = (trackEvent.offsetHeight +
                                TRACK_PADDING) * track.order + "px";
                        }
                    }
                }
            });
